...
If the DAG is expected to edit the runner itself (`.cursor/skills/dag-task-runner/scripts/**`), launch through the supervisor instead so source edits take effect at a process boundary:

```bash
"$RUNNER_DIR/node_modules/.bin/tsx" "$RUNNER_DIR/run_dag_supervisor.ts" \
  --dag /tmp/dag-<slug>.json \
  --canvas-path "$CANVAS_PATH" \
  --state-path "$HOME/.cursor/projects/<workspace-slug>/dag-state/<slug>.json"
```

The supervisor passes `--restart-on-runner-change` to `run_dag.ts`. When runner runtime files change after a rank or convergence iteration, the child runner persists state, marks the canvas `RESTARTING RUNNER`, exits `75`, and the supervisor relaunches with `--resume-state` so pending tasks continue under the new source.
...
| `--state-path`               | —                   | Persist resumable runner state. Defaults to `.dag-runner/run-state.json` when `--restart-on-runner-change` is set. |
...
| `--max-runner-restarts`      | `20`                | Supervisor-only cap for relaunches from `run_dag_supervisor.ts`.                                                   |
