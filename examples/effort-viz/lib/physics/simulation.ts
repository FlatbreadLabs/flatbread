...
export function createGraphSimulation(options: SimulationOptions = {}): GraphSimulation {
  ...
  function spawnNode(input: GraphInputNode, parent: InternalNode | null): InternalNode {
    ...
    const dist = randRange(rand, parentR * 0.6, parentR * (0.6 + opts.spawnJitter));
  ...
  function sync(inputNodes: GraphInputNode[], inputEdges: GraphInputEdge[]): void {
    ...
    pending.sort((a, b) => Number(b.kind === 'effort') - Number(a.kind === 'effort'));
    ...
    for (const n of nodes) if (!n.present && n.state !== 'retracting') n.state = 'retracting';
    for (const e of edges) if (!e.present && e.state !== 'retracting') e.state = 'retracting';
  ...
  function refreshVeinPaths(): void {
    for (const edge of edges) {
      ...
      if (!from || !to) continue;
      buildVeinPath(from.x, from.y, to.x, to.y, edge.offsets, opts.veinNoise, edge.path);
    }
  }
