...
import type { ProofMutation } from '@flatbread/proof';
...
test.serial(
  ...
  async (t) => {
    ...
    const result = await server.proof!.writer.mutate({
      type: 'CreateEffort',
      title: 'Committed effort',
      body: '',
    });
    await server.proof!.waitForCommittedGeneration(result.generation);
...
test.serial(
  ...
  async (t) => {
    ...
    const created = await server.proof!.writer.mutate({
      type: 'CreateEffort',
      title: 'GraphQL without explorer assets',
      body: '',
    });
    await server.proof!.waitForCommittedGeneration(created.generation);
    const response = await query(server.port, '{ allEfforts { title } }');
    t.deepEqual(response.errors, undefined);
...
test.serial(
  'inactive config exposes no proof and behaves as before',
  async (t) => {
    const fixture = await makeDir();
    t.teardown(() => rm(fixture.dir, { recursive: true, force: true }));
    const server = await startGraphqlServer({
...
test.serial(
  ...
  async (t) => {
    ...
    const effort = await server.proof!.writer.mutate({
      type: 'CreateEffort',
      title: 'GraphQL cite chain',
      body: '',
    });
    const effortId = effort.artifacts[0].id;
    const otherEffort = await server.proof!.writer.mutate({
      type: 'CreateEffort',
      title: 'Other chain',
      body: '',
    });
    const otherEffortId = otherEffort.artifacts[0].id;
    const blob = await server.proof!.writer.mutate({
      type: 'WriteBlob',
      effort: effortId,
      title: 'Payload',
    ...
    const citation = await server.proof!.writer.mutate({
      type: 'WriteCitation',
      effort: effortId,
      title: 'Paper',
    ...
    await server.proof!.waitForCommittedGeneration(citation.generation);
    ...
    const foreignBlob = await server.proof!.writer.mutate({
      type: 'WriteBlob',
    ...
    const foreignCitation = await server.proof!.writer.mutate({
      type: 'WriteCitation',
      effort: otherEffortId,
      title: 'Foreign paper',
    ...
    const finding = await server.proof!.writer.mutate({
      type: 'WriteFinding',
      effort: effortId,
      title: 'Measured',
      body: 'short',
    ...
    await server.proof!.waitForCommittedGeneration(finding.generation);
...
test.serial(
  ...
  async (t) => {
    ...
    const snapshot = await server.proof!.waitForCommittedGeneration('7');
