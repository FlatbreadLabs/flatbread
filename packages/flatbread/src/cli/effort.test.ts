...
test('public flatbread facade exposes the Effort Graph preset', (t) => {
  t.deepEqual(publicEffortGraphContent(), effortGraphContent());
});
...
test.serial(
  ...
  async (t) => {
    ...
    const result = await runCli(cwd, 'effort', 'bootstrap', '--verify');
    t.not(result.code, 0);
...
test.serial(
  ...
  async (t) => {
    for (const args of [
      ['effort', 'list', '--status', 'invalid'],
      ['effort', 'list', '--limit', '0'],
      ['effort', 'list', '--strict-min-generation', '1.5'],
    ]) {
      const result = await runCli(process.cwd(), ...args);
      t.not(result.code, 0);
      const payload = JSON.parse(result.stderr);
      t.regex(payload.error.code, /^EFFORT_GRAPH_INVALID_/);
...
export default {
  ...
  content: effortGraphContent(${JSON.stringify(
    relative(cwd, join(cwd, '.flatbread-efforts'))
  )}),
};`
    );
...
import { transformer } from '@flatbread/transformer-markdown';
import { effortGraphContent } from '@flatbread/effort-graph';
export default { source: source(), transformer: transformer(), content: effortGraphContent() };`
    );
...
import { transformer } from '@flatbread/transformer-markdown';
import { effortGraphContent } from '@flatbread/effort-graph';
export default { source: source(), transformer: transformer(), content: effortGraphContent('memory/graph') };`
    );
    await writeFile(
      join(cwd, '.gitignore'),
