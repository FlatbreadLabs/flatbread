...
async function rootFor(cwd: string): Promise<string> {
  ...
  const root = findEffortGraphContentRoot(loaded.config?.content ?? []);
  if (!root)
    throw new Error(
      'No complete effort-graph preset found in flatbread.config; add effortGraphContent() to content.'
    );
...
export async function handleEffortGet(
  ...
  return getRecord(id, {
    ...
    cacheRoot: resolve(cwd, '.flatbread/effort-graph'),
...
export async function inspectEffortBootstrap(
  ...
  const graphRoot =
    findEffortGraphContentRoot(loaded.config?.content ?? []) ??
    DEFAULT_GRAPH_ROOT;
  const requirements: EffortBootstrapReport['requirements'][number][] = [];
  if (!findEffortGraphContentRoot(loaded.config?.content ?? []))
    requirements.push(
      requirement(
  ...
  if (
    ...
    requirements.push(
      requirement(
        ...
        `The Effort Graph journal for ${graphRoot} is not ignored.`,
  ...
  if (
    ...
    requirements.push(
      requirement(
        ...
        'The derived Effort Graph read cache is not ignored.',
        'Add **/.flatbread/effort-graph/read-cache/ to .gitignore.'
      )
    );
  return {
...
export function registerEffortCommands(prog: any): void {
  const hasShape = (
    value: unknown
    ...
    try {
      ...
      if (hasShape(error)) payload = error.shape;
        ...
        try {
          ...
          payload = {
            error: { code: 'EFFORT_CLI_ERROR', message: error.message },
          };
        }
      } else {
        payload = {
          error: { code: 'EFFORT_CLI_ERROR', message: String(error) },
        };
      }
  ...
  prog
    .command('effort get <id>', 'Read one Effort Graph record')
    .option(
  ...
  prog
    .command(
      'effort blocking-decisions <effortId>',
  ...
  recordsOptions(
    prog.command('effort records <effortId>', 'Read records in an Effort')
  ).action(async (effortId: string, options: Record<string, unknown>) =>
    printResult(handleEffortRecords(effortId, mapEffortCliOptions(options)))
  );
  paginationOptions(
    consistencyOptions(
      prog.command('effort list', 'List Efforts by lifecycle status')
    )
  )
  ...
  prog
    .command('effort bootstrap', 'Inspect Effort Graph activation requirements')
  ...
  paginationOptions(
    consistencyOptions(
      prog.command(
        'effort relations <effortId> <fromId>',
  ...
  prog
    .command('effort cache prune', 'Prune derived read cache')
    .action(async () =>
      printResult(
        pruneReadCache(resolve(process.cwd(), '.flatbread/effort-graph'))
      )
    );
}
