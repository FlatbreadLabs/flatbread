...
export function verifyReleaseIdentity(canonicalTexts, packageVersions) {
  ...
  if (
    ...
    release.proofVersion !== packageVersions.proofVersion ||
...
export function verifyPackPayload(
  ...
  if (missing.length > 0) {
    throw new Error(
      [
        'Proof package payload is missing canonical skill files:',
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  verifyPackagePayload()
    .then((files) => {
      console.log(
        `Proof package payload verified (${files.length} skill files).`
      );
    })
    .catch((error) => {
