...
import {
  ProofGenerationWaitTimeoutError,
  ProofLiveSchemaRejectedError,
...
export interface ProofLiveBridge {
  readonly rootDir: string;
  readonly writer: ProofWriter;
...
export interface CreateProofLiveBridgeOptions
  extends Omit<ProofWriterOptions, 'publisher'> {
...
export async function createProofLiveBridge(
  options: CreateProofLiveBridgeOptions
): Promise<ProofLiveBridge> {
  ...
  const publisher: CommittedGenerationPublisher = {
    async publish(publication: CommittedGenerationPublication) {
      ...
      if (result.status === 'rejected') {
        throw new ProofLiveSchemaRejectedError(publication, result.error);
      }
      liveGenerations.set(
  ...
  async function waitForCommittedGeneration(
    ...
    const sleep = async () => {
      ...
      if (remaining <= 0)
        throw new ProofGenerationWaitTimeoutError(
          token,
