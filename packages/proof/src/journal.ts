  ...
  for (const txn of txns) {
    ...
    if (!committed) {
      ...
      try {
        ...
        throw new ProofReindexFailedError('Recovery reindex failed', e);
      }
...
export async function commitJournal(
  ...
  for (const w of [...writes].sort((a, b) =>
    ...
    if (!(await verifyLock()))
      throw new ProofLockedError(0, 'Writer lock lost mid-transaction');
  ...
  try {
    ...
    throw new ProofReindexFailedError('Reindex failed', e);
  }
