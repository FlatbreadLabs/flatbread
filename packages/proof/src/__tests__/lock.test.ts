...
import { ProofLockedError, ProofValidationError } from '../errors.js';
...
test('second concurrent acquire throws ProofLockedError', async (t) => {
  const root = await makeRoot();
  const lock = await acquireWriterLock(root);
  const error = (await t.throwsAsync(acquireWriterLock(root), {
    instanceOf: ProofLockedError,
  })) as ProofLockedError;
  t.is(error.code, 'PROOF_LOCKED');
...
test('lock is released after a failed mutate so the next mutate succeeds', async (t) => {
  ...
  const writer = createProofWriter({ rootDir: root });
  await t.throwsAsync(
    writer.mutate({
