...
import { createProofLiveBridge } from '../live.js';
import {
  ProofGenerationWaitTimeoutError,
  ProofReindexFailedError,
} from '../errors.js';
...
async function makeBridge() {
  ...
  const bridge = await createProofLiveBridge({
    rootDir: root,
...
test.serial(
  ...
  async (t) => {
    ...
    await t.throwsAsync(
      ...
      { instanceOf: ProofReindexFailedError }
    );
...
test.serial(
  ...
  async (t) => {
    ...
    await t.throwsAsync(
      ...
      {
        instanceOf: ProofGenerationWaitTimeoutError,
      }
    );
  }
);
...
test.serial(
  ...
  async (t) => {
    ...
    const recreated = await createProofLiveBridge({
      rootDir: root,
      reloader: fake.reloader,
    });
    t.is(
