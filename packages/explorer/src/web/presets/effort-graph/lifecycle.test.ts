...
function node(
  partial: Partial<GraphNode> & Pick<GraphNode, 'id' | 'kind'>
): GraphNode {
  return {
    title: partial.id,
    effortId: 'eff-1',
...
describe('effectiveLifecycle', () => {
  test('reports a superseded Decision as retired even though it says accepted', () => {
    ...
    const index = buildLifecycleIndex([
      edge('superseded_by', 'dec-old', 'dec-new'),
    ]);
  ...
  test('derives supersession from the newer record’s forward edge too', () => {
    ...
    const index = buildLifecycleIndex([
      edge('supersedes', 'dec-new', 'dec-old'),
    ]);
  ...
  test('classifies frontmatter states without edges', () => {
    ...
    for (const [record, expected] of cases) {
      assert.equal(
        effectiveLifecycle(record, index).aliveness,
        expected,
        record.id
      );
    }
...
describe('isOpenBlocker', () => {
  test('only flags open Issues whose kind is blocker', () => {
    ...
    assert.equal(
      isOpenBlocker(blocker, effectiveLifecycle(blocker, index)),
      true
    );
    assert.equal(isOpenBlocker(gap, effectiveLifecycle(gap, index)), false);
    assert.equal(
      isOpenBlocker(closed, effectiveLifecycle(closed, index)),
      false
    );
    assert.equal(
      isOpenBlocker(deferred, effectiveLifecycle(deferred, index)),
      false
    );
  });
});
