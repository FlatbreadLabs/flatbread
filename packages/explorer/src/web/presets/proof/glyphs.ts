 ...
 */
export const GLYPH_OUTLINES: Record<
  Exclude<GlyphId, 'circle' | 'ring'>,
  GlyphPoint[]
> = {
...
export function glyphSvgPoints(glyph: GlyphId, size: number): string | null {
  ...
  const extent = Math.max(
    ...outline.map((p) => Math.max(Math.abs(p.x), Math.abs(p.y)))
  );
  const half = size / 2;
  ...
  return outline
    .map(
      ({ x, y }) =>
        `${(half + x * scale).toFixed(2)},${(half - y * scale).toFixed(2)}`
    )
    .join(' ');
}
