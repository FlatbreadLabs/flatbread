...
import { resolveEventsUrl, resolveGraphqlEndpoint } from '../../core/endpoints';
import { graphqlFetch } from '../../core/graphql';
import { normalizeProof } from './normalize';
import {
  SCHEMA_PROBE_QUERY,
  buildProofQuery,
  type ProofQueryResult,
...
/**
 * Live connection / query state for the Proof viz.
 *
...
export interface UseProofLiveOptions {
  endpoint?: string;
}

export interface UseProofLiveResult {
  nodes: GraphNode[];
  edges: GraphEdge[];
...
export function shouldCommitGeneration(
  ...
  return committedGeneration === null || nextGeneration >= committedGeneration;
}
...
export function useProofLive(
  options: UseProofLiveOptions = {}
): UseProofLiveResult {
  const endpoint = options.endpoint ?? resolveGraphqlEndpoint();
  const eventsUrl = resolveEventsUrl(endpoint);
  ...
  const fetchGraph = useCallback(async (): Promise<FetchedGraph> => {
    ...
    try {
      ...
      // buildProofQuery takes the scalars-only safe path.
    }

    const schema = resolveSchemaForQuery(probed, lastGoodSchemaRef.current);
    const data = await graphqlFetch<ProofQueryResult>(
      buildProofQuery(schema),
      undefined,
      endpoint
    );
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      source.addEventListener('ready', (event) => {
        void maybeRefetch(
          parseGeneration((event as MessageEvent<string>).data)
        );
      });
...
export { graphqlOrigin } from '../../core/endpoints';
