import type { ProofQueryResult, GraphRef } from './query';
import type {
  Proof,
  GraphEdge,
  GraphEdgeKind,
...
function relationIds(value: unknown): string[] {
  ...
  if (
    value &&
    typeof value === 'object' &&
    typeof (value as GraphRef).id === 'string'
  ) {
...
function addRecordNode(
  ...
  const effortId = effortIdFromRef(
    record.effort as GraphRef | null | undefined
  );
...
export function normalizeProof(data: ProofQueryResult): Proof {
