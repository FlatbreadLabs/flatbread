...
import { buildProofQuery, type SchemaProbeResult } from './query';
...
describe('buildProofQuery', () => {
  test('includes Risk supersedes/superseded_by when schema lists them', () => {
    ...
    const risks = collectionBlock(buildProofQuery(schema), 'allRisks');
  ...
  test('with null schema does not emit the full RELATION_FIELDS catalog', () => {
    ...
    const query = buildProofQuery(null);
