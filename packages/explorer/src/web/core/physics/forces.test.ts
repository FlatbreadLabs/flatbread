...
describe('computeClusters', () => {
  ...
  test('member buckets cover every node exactly once', () => {
    ...
    for (let s = 0; s < clusters.count; s++) {
      for (
        let k = clusters.memberStart[s];
        k < clusters.memberStart[s + 1];
        k++
      ) {
  ...
  test('member counts survive the bucketing pass', () => {
    ...
    computeClusters(nodes, scratch);
    assert.equal(
      scratch.clusters.members[scratch.clusters.indexOf.get('A')!],
      2
    );
    assert.equal(
      scratch.clusters.members[scratch.clusters.indexOf.get('B')!],
      1
    );
  });
});
