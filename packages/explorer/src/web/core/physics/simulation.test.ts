...
function stepFor(
  sim: ReturnType<typeof createGraphSimulation>,
  seconds: number,
  dt = 1 / 60
) {
...
describe('createGraphSimulation', () => {
  ...
  it('records spawn near their effort hub', () => {
    ...
    for (const rec of state.nodes.filter((n) => n.kind === 'record')) {
      ...
      assert.ok(
        d <= hub.radius * (0.6 + 0.5) + 1e-6,
        `record ${rec.id} spawned too far (${d})`
      );
    }
  });
  ...
  it('re-adding a retracting node revives it', () => {
    ...
    sim.step(0.1);
    assert.equal(
      sim.getState().nodes.find((n) => n.id === 'rec-1')!.state,
      'retracting'
    );
    sim.sync(nodes, edges);
  ...
  it('displaces overlapping neighbors when a new node is packed in', () => {
    ...
    stepFor(sim, 2);
    const rec1Before = {
      ...sim.getState().nodes.find((n) => n.id === 'rec-1')!,
    };
    ...
    const drift = Math.hypot(
      rec1After.x - rec1Before.x,
      rec1After.y - rec1Before.y
    );
    assert.ok(
      drift > 0,
      'existing record should be nudged by the new arrivals'
    );
  });
