...
> Companion to [`flatbread-agent-artifact-opportunity.md`](flatbread-agent-artifact-opportunity.md) and [`flatbread-flow-pmf-audit.md`](flatbread-flow-pmf-audit.md). Authored via a `/proof` DAG; intermediate dossier names were folded into this report and are not checked into the repo. Evidence for claims is in repo paths cited inline and in **§13 — References** (no ephemeral `file://` paths).
...
The internal Flatbread codebase audit (sections cited here match the audit dossier §2.4–2.5, §3.1, §5) is unambiguous about today's retrieval surface in [`packages/core`](packages/core):
